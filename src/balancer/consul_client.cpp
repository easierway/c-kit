...
std::tuple<int, std::vector<std::shared_ptr<ServiceNode>>, std::string> ConsulClient::GetService(const std::string& serviceName, int timeoutS, std::string& lastIndex) {
    ...
    for (const auto& service : jsonObj.array_items()) {
        ...
        if (!service["Service"]["Meta"]["machine"].is_null()) {
            machine = service["Service"]["Meta"]["machine"].string_value();
        }
...
std::tuple<int, json11::Json, std::string> ConsulClient::GetKV(const std::string& path, int timeoutS, std::string& lastIndex) {
    std::vector<std::shared_ptr<ServiceNode>> nodes;
    std::string                               body;
    int                                       status = -1;
    std::string                               err;
